...
static void test_cmt_keccak_init(void) {
    cmt_keccak_t state;
    cmt_keccak_init(&state);
    printf("Test cmt_keccak_init: Passed\n");
}

static void test_cmt_keccak_hash_operations(void) {
...
static void test_cmt_keccak_large_update(void) {
    // 200 bytes (exceeds rsiz=136) to trigger keccakf mid-stream
    const char input[200] = {[0 ... 199] = 'A'};
    uint8_t result[CMT_KECCAK_LENGTH] = {0};
    uint8_t expected[CMT_KECCAK_LENGTH] = {0};

    cmt_keccak_data(sizeof(input), input, expected);

    cmt_keccak_t state;
    cmt_keccak_init(&state);
    cmt_keccak_update(&state, sizeof(input), input);
    cmt_keccak_final(&state, result);

    assert(memcmp(result, expected, CMT_KECCAK_LENGTH) == 0);
    printf("Test cmt_keccak_large_update: Passed\n");
}

static void test_cmt_keccak_funsel(void) {
